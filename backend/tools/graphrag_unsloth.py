...
async def extract_entities(text_units: pd.DataFrame,
    ...
    model_name = "unsloth/qwen2.5-7b-instruct-bnb-4bit", # Model đã được tối ưu 4-bit
    max_seq_length = 2048,
    load_in_4bit = True,
)
    FastLanguageModel.for_inference(model) # Tối ưu hóa cho việc suy luận

    # 2. Thay đổi hàm call_gpt
    async def call_gpt(text: str, system_prompt, user_content) -> str:
        # Format tin nhắn theo chuẩn của model (ví dụ Qwen hoặc Llama)
        messages = [
        ...
        return response[0].split("<|im_start|>assistant")[-1].replace("<|im_end|>", "").strip()
