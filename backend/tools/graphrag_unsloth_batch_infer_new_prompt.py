...
ENTITY_TYPES = (
    "VĂN BẢN, ĐIỀU KHOẢN, CHỦ THỂ, QUYỀN & NGHĨA VỤ, "
...
Nhiệm vụ của bạn là trích xuất một Đồ thị tri thức (Knowledge Graph) sâu sắc, tập trung vào việc kết nối các thực thể để phục vụ việc tra cứu luật.

-QUY TẮC NGỮ CẢNH-
Văn bản nguồn: {{doc_name_context}}

-CÁC BƯỚC THỰC HIỆN-

1. XÁC ĐỊNH THỰC THỂ (ENTITIES):
Trích xuất các thực thể dựa trên danh sách sau:
- VĂN BẢN: Tên bộ luật hoặc văn bản quy phạm liên quan (ví dụ: BỘ LUẬT HÌNH SỰ 2015).
- ĐIỀU KHOẢN: Tên Điều, Khoản, Điểm. QUY TẮC: Phải đính kèm mã hiệu văn bản, ví dụ: "KHOẢN 1 ĐIỀU 5 ({{doc_name_context}})".
- CHỦ THỂ: Cơ quan, tổ chức, cá nhân có quyền hoặc trách nhiệm (ví dụ: BỘ CÔNG AN, NGƯỜI PHẠM TỘI).
- QUYỀN & NGHĨA VỤ: Các quyền được làm hoặc nghĩa vụ phải làm.
- HÀNH VI VI PHẠM: Mô tả hành vi bị cấm hoặc vi phạm pháp luật.
- HẬU QUẢ PHÁP LÝ: Hình phạt, biện pháp khắc phục, mức xử phạt.
- ĐIỀU KIỆN ÁP DỤNG: Tình huống cụ thể để luật có hiệu lực (ví dụ: TRƯỜNG HỢP THIÊN TAI).
- THỜI HẠN/THỜI HIỆU: Các mốc thời gian quy định (ví dụ: 15 NGÀY LÀM VIỆC).

Định dạng: ("entity"<|><entity_name><|><entity_type><|><entity_description>)

2. XÁC ĐỊNH MỐI QUAN HỆ (RELATIONSHIPS):
Đây là bước quan trọng nhất để tránh phân mảnh đồ thị. Hãy trích xuất:
- Cấu trúc: (Khoản/Điểm) -> [THUỘC VỀ] -> (Điều) -> [THUỘC VỀ] -> (Văn bản).
- Dẫn chiếu (CỰC KỲ QUAN TRỌNG): (Điều A) -> [DẪN CHIẾU TỚI/CĂN CỨ VÀO] -> (Điều B hoặc Văn bản khác).
- Đặc thù: (Điều A) -> [LOẠI TRỪ/TRỪ TRƯỜNG HỢP] -> (Điều B).
- Chế tài: (HÀNH VI VI PHẠM) -> [DẪN ĐẾN] -> (HẬU QUẢ PHÁP LÝ).
- Thẩm quyền: (CHỦ THỂ) -> [CÓ QUYỀN/CÓ TRÁCH NHIỆM] -> (ĐIỀU KHOẢN).

Định dạng: ("relationship"<|><source_entity><|><target_entity><|><description><|><strength>)

3. ĐỊNH DẠNG ĐẦU RA:
- Các phần tử cách nhau bởi dấu ##. Ngôn ngữ: TIẾNG VIỆT hoàn toàn.
- Kết thúc bằng: <|COMPLETE|>

-VÍ DỤ MẪU-
("entity"<|>ĐIỀU 10 (LUẬT X)<|>ĐIỀU KHOẢN<|>Quy định về hành vi trốn thuế)
##
("entity"<|>PHẠT TIỀN TỪ 10 TRIỆU ĐẾN 20 TRIỆU<|>HẬU QUẢ PHÁP LÝ<|>Mức xử phạt hành chính đối với hành vi trốn thuế lần đầu)
##
("relationship"<|>ĐIỀU 10 (LUẬT X)<|>PHẠT TIỀN TỪ 10 TRIỆU ĐẾN 20 TRIỆU<|>Vi phạm quy định tại Điều 10 dẫn đến mức phạt tiền tương ứng<|>9)
<|COMPLETE|>
"""
...
async def extract_entities(text_units: pd.DataFrame,
    ...
    batch_size: int = 4, # Số lượng chunk xử lý trong một batch
...
async def main():
    ...
    # for index, text_df in tqdm(law_texts_df.iterrows(), total=len(law_texts_df), desc="Chunking law texts"):
    #     print(f"text_df: {text_df}")
    #     df_chunks = chunk_civil_code_markdown(text_df["content"])
    #     df_chunks_final = pd.concat([df_chunks_final, df_chunks], ignore_index=True)
    # #df_chunks = chunk(raw_text, chunk_size=50, chunk_overlap=10)
    # print(f"df_chunks: {df_chunks_final}")
